...
<!---->
    ...
    <div id="body">
        <div id="content">
            <div id="fixItApp">
				<div id="popupCont" class={`${popupOpen ? 'visible' : ''}`}>
					<h2>Create Todo Item</h2>
					<form>
  <input type="text" id="todo-title" bind:value={title} placeholder="Todo title" />
  <input type="text" id="todo-description" bind:value={description} placeholder="Todo description" />
  <input type="text" id="todo-dueDate" bind:value={dueDate} placeholder="Todo dueDate" />
  <button on:click={() => createTodo(title, description, dueDate)}>Create</button>
</form>
				</div>
                <div id="todoList">
					
					<button on:click={() => openPopup()} class="addBtn">Add</button>
